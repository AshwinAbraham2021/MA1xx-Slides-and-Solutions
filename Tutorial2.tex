...
\begin{document}
        ...
        \item Let $\Lim{x\to \alpha}f(x) = L$
                ...
                Thus the limit exists.\footnote{One could have also argued the same using algebra of limits on functions.}

                The converse of the result would be that if $\Lim{h\to 0} f(\alpha+h) - f(\alpha-h) = 0$, then $\Lim{x\to \alpha}f(x)$ exists.
                ...
                $$f(x) = \begin{cases}
                        ...
                        0  & \text{ for } x =  0\\
                \end{cases}$$
                Take $\alpha = 0$, $f(0+h)-f(0-h) = 0$ for all $h$, since this is an even function, but $\Lim{x\to 0} f(x)$ does not exist.
        \item \begin{enumerate}
                \item 
                        $$f(x) = \begin{cases}
                            ...
                            The limit of a function $f$ at $a$ exists and is equal to $L$ if and only if for every sequence ${x_n} \to a$, $\Lim{n\to \infty}f(x_n) = L$

                            Take ${x_n} = \frac{1}{2n\pi}$ and $y_n = \frac{1}{(2n+\frac{1}{2})\pi}$. We have that $\sin{x_n} = 0$ and $\sin{y_n} = 1$ for all n $\in \mathbb{N}$
...
\item For $x \neq 0$, $\frac{1}{x}$ is differentiable while $\sin x$ and $x$ are differentiable at all points. Hence, a function formed by their compositions and products would also be differentiable at $x \neq 0$. At $x = 0$, we'll need to work by first principles.
        ...
        \item First, note that if $\exists c  \text{ such that } f(c) = 0$, then $f(x) = f(x-c+c) =f(x-c)f(c) = 0\, \forall x \in \mathbb{R}$. In this case f is obviously differentiable, with $f'(c) = 0 \forall c = f'(0)f(c)$ (trivially).
                ...
                Since $f'(0) = \Lim{h\to 0}\frac{f(h) - 1}{h}$ exists, we have that $\Lim{h\to 0}\frac{f(c+h)-f(c)}{h} =  \Lim{h\to 0} f(c)\times \frac{f(h) - 1}{h}$ exists and is equal to $f(c)\times \Lim{h\to 0}\frac{f(h)-f(0)}{h} = f(c)f'(0)$. Thus, by definition, $f$ is differentiable at every $c\in \mathbb{R}$ and $f'(c) = f(c) f'(0)$

        \item Calculating derivative of inverse of a function. Let $f: (a, b) \to \mathbb{R}$ be a differentiable and invertible function. Let the inverse be denoted by $g(x)$, which means that 
