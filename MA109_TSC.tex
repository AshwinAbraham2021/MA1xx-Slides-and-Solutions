...
\title{MA 109 TSC I: Concept Recap}
\author[Aryaman Maithani]{\texorpdfstring{Aryaman Maithani\\\url{https://tinyurl.com/MA109-2022}}{Aryaman Maithani}}
\date{Autumn Semester {\color{red}2022}-23}
...
\begin{frame}{Week 3}
    What have we seen so far? \uncover<2->{Continuity, }\uncover<3->{IVP, }\uncover<4->{EVT, }\uncover<5->{sequential criterion, }\uncover<6->{derivative, }\uncover<7->{(local) maximum and minimum, }\uncover<8->{Fermat's not-Last Theorem, }\uncover<9->{Rolle's and Mean Value Theorems, }\uncover<10->{Darboux's Theorem. }\hfill\uncover<11->{{\color{gray}(Whew!)} }
...
%     \uncover<7->{In fact, a critical point which is not a point of local extremum is called a {\color{blue}saddle point}. }\uncover<8->{\hfill (Definition.) }

%     \uncover<9->{In the one-variable case, we did have a second derivative test which let us conclude more information. }\uncover<10->{We have a similar thing here. }
% \end{frame}
% \begin{frame}{Week 6}
...
%             \item If $D > 0$ and $f_{xx}(x_0, y_0) > 0,$ then $(x_0, y_0)$ is a local minimum for $f.$
%             \uncover<6->{\item If $D > 0$ and $f_{xx}(x_0, y_0) < 0,$ then $(x_0, y_0)$ is a local maximum for $f.$ }
...
% \end{frame} 
% \begin{frame}{Week 6}
%     Some things regarding the second derivative test.
%     \begin{enumerate}
%         \uncover<2->{\item If you get $D = 0$ in the last test, you would have to analyse the function on your own and try to find out the behaviour. }\uncover<3->{(We shall do this in a tutorial question today.) }
%         \uncover<4->{\item Note that, by definition, if a critical point is not a local extremum, then it must be a saddle point. }\uncover<5->{In other words, a critical point is either a point of local extremum or a saddle point. }
%     \end{enumerate}
% \end{frame}
% \begin{frame}{Week 6}
